...
class Client : noncopyable {
   public:
    ...
    Client(const string& name, EventLoop* loop, const InetAddress& serverAddr,
           Operation op, int requests, int keys, int valuelen,
   ...
   private:
    ...
    void onMessage(const TcpConnectionPtr& conn, Buffer* buffer,
        ...
        if (op_ == kSet) {
            ...
            while (buffer->readableBytes() > 0) {
                const char* end = static_cast<const char*>(memmem(
                    buffer->peek(), buffer->readableBytes(), "END\r\n", 5));
                if (end) {
...
int main(int argc, char* argv[]) {
    ...
    double memoryMiB =
        1.0 * clients * keys * (32 + 80 + valuelen + 8) / 1024 / 1024;
    ...
    connected.wait();
    LOG_WARN << clients << " clienets all connected";
